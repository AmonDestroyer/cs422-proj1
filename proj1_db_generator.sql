...
CREATE TABLE IF NOT EXISTS `proj1_db`.`TS_Set` (
  ...
  `max_length` INT NULL,
  `num_ts` INT NULL,
  `Time Unit_tu_id` INT NOT NULL,
  PRIMARY KEY (`set_id`),
  INDEX `fk_TS_Set_Time Unit1_idx` (`Time Unit_tu_id` ASC) VISIBLE,
  CONSTRAINT `fk_TS_Set_Time Unit1`
    FOREIGN KEY (`Time Unit_tu_id`)
...
CREATE TABLE IF NOT EXISTS `proj1_db`.`TS_Measurement` (
  ...
  `x_val` INT NULL,
  `y_val` DOUBLE NULL,
  `Time Series_ts_id` INT NOT NULL,
  PRIMARY KEY (`tsm_id`),
  INDEX `fk_TS_Measurement_Time Series1_idx` (`Time Series_ts_id` ASC) VISIBLE,
  CONSTRAINT `fk_TS_Measurement_Time Series1`
    FOREIGN KEY (`Time Series_ts_id`)
    REFERENCES `proj1_db`.`Time Series` (`ts_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
